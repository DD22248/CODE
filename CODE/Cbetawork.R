Cbetawork <- function(wdf1,c,m,T0,edis,clN){
  q <- 3
  n <- m^2*T0
  u <- c()
  v <- c()
  ...
  for (i in 1:n){
    ...
    t[i] <- floor((i-1)/m^2)+1
    rho1[i] <- 0.3+0.6*(v[i]^2-u[i]^2)*(t[i]/T0) 
    rho2[i] <- -0.3+0.6*sin(pi*(u[i]^2+v[i]^2+(t[i]/T0)^2))  
    beta[i,1] <- (u[i]+v[i])*exp(t[i]/10)
    beta[i,2] <- 32*u[i]*v[i]*(u[i]-1)*(v[i]-1)*t[i]*(t[i]-2)
    beta[i,3] <- -2-c*cos(pi*v[i]*exp(t[i]/4))     
  }
  ...
  f <- function(N){
    ...
    time1 <- system.time(
      outGL1 <- Cbetatest(W,Q,Xc,Xv,X,Y1,K0,ds,Ht,dt,B)
    )
    time2 <- system.time(
      outLL1 <- CbetatestLL(W,Q,u,v,t,Xc,Xv,X,Y1,K0,ds,Ht,dt,B)
    )
    out1 <- cbind(outGL1,outLL1)
    outGL2 <- Cbetatest(W,Q,Xc,Xv,X,Y2,K0,ds,Ht,dt,B)
    outLL2 <- CbetatestLL(W,Q,u,v,t,Xc,Xv,X,Y2,K0,ds,Ht,dt,B)
...
out <- list()
for (i in 1:cn){
  test[[i]] <- Cbetawork(wdf1,c00[i],25,4,edis=1,1:N)
  Bp[[i]] <- test[[i]][[1]]$out[3,]
  for(j in 2:N){
